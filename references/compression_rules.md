...
把长文档翻译成视觉沟通规格——每个概念不只有文字，还应指定视觉形式和数据锚点。

压缩的终点不是”更短的文字”，而是”可以被直接视觉化的页面规格”。如果压缩完的 clean pages 只有文字面板而没有图表声明、icon 指定、数据锚点，说明压缩还没做到位。

## 核心规则

1. 一页只保留一个主结论
2. **每个可视化的概念都应标注视觉形式**——不只是”能变图形的不保留文字”，而是”必须指定变成什么图形”
3. 删除内部讨论语气
4. 删除重复推导和连接词
5. 不保留”为了完整而完整”的说明
6. **为缺口/程度类概念生成说明性数据锚点**（参见 `illustrative_data_guide.md`）
