...
每一页都有视觉主角（图表、icon 链、仪表盘、架构图、概念化 UI），不能只放文字面板。压缩步骤的目标是把商业逻辑翻译成视觉沟通规格。
...
Step 1.4b★ 长篇治理                           → section_packages.json / 高密度原型
...
长篇 Expert Deck 还必须通过更严格的长篇治理门禁：

- `deck_capacity_plan.json.budget_tiers` 包含 conservative、recommended、extended、appendix_heavy 四档
- `section_packages.json` 把所有正文页分配到章节
- 高密度页使用 `references/dense_page_archetypes.md` 中的已知原型
- 章节写作使用 `section-handoff --section-id <id>`，让每个 AI 只接收当前章节上下文
...
Build AI 把这份规格作为一等输入：图表类型、数据值、icon 和布局比例必须清楚，不能只写"三张诊断卡"这类粗略描述。
...
3. **AI 带假设提问**——问题应写成"我的判断是 X，你觉得对吗？"，避免泛泛询问"你有什么补充"
