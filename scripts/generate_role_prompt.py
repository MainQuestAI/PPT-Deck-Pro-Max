...
def build_review_prompt(project_dir: Path, review_package_path: Path | None = None) -> str:
    ...
    return "\n".join(
        [
            ...
            "- `slide_state.json`",
            "- `montage.png`（如果存在）",
            "- `deck_source_digest.md`、`deck_claim_map.json`、`deck_capacity_plan.json`、`deck_gap_registry.json`（如果 `review_package.json.content_governance_summary.enabled=true`，这几份必须检查）",
            "- `section_packages.json`、`deck_section_packages.md`、`references/dense_page_archetypes.md`（如果 `review_package.json.longform_governance_summary.enabled=true`，这几份必须检查）",
            ...
            "- 先读取 `review_package.json` 中的 `content_governance_summary`；如果存在 `capacity_over_target` 或 `blocking_gaps`，必须先给出内容门禁 finding",
            "- 先读取 `review_package.json` 中的 `longform_governance_summary`；如果存在 `budget_requires_extension_strategy`、`section_page_total_mismatch`、`forbidden_topic_conflict` 或 `dense_archetype_missing`，必须先给出长篇治理 finding",
            ...
            "- 如果 `expert_mode_summary.gating_status.finalized=false`、`redaction_pending>0`、`coverage_target_met=false` 或 `expert_context_ready=false`，必须给出对应 finding",
